...
from alm.motion_completion._condmdi.diffusion.nn import mean_flat, sum_flat
from alm.motion_completion._condmdi.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood
from alm.motion_completion._condmdi.data_loaders.humanml.scripts import motion_process
from torch.cuda import amp
from typing import List
import alm.motion_completion._condmdi.utils.editing_util as inpainting_util
    ...
    def p_mean_variance(
        ...
        if inpainting_util.requires_reconstruction_guidance(model_kwargs, t):
            ...
            if model_kwargs['y']['replacement_distribution'] == 'conditional':
                ...
                raise NotImplementedError
        elif inpainting_util.requires_alm(model_kwargs, t):
            text_scale_temp = model_kwargs['y']['text_scale'].detach().clone()
            model_kwargs['y']['text_scale'] = torch.ones(text_scale_temp.shape[0], device=text_scale_temp.device)
            model_output_cond = get_conv_model_output(x, t, model_kwargs)
            ...
            eps_tgt = self._predict_eps_from_xstart(x_t=x, t=t, pred_xstart=process_xstart(model_output_cond.clone(), self.model_mean_type))
            assert x.shape == eps_tgt.shape == model_output_cond.shape

            source_motion_x_t = self.q_sample(x_start=inpainted_motion * inpainting_mask, t=t)
            source_motion_e_t = source_motion_x_t * inpainting_mask + x * ~inpainting_mask
            model_output_src_e_t = get_conv_model_output(source_motion_e_t, t, model_kwargs)
            eps_e = self._predict_eps_from_xstart(x_t=source_motion_e_t, t=t, pred_xstart=process_xstart(model_output_src_e_t, self.model_mean_type))
            assert source_motion_e_t.shape == eps_e.shape

            delta_x = model_kwargs['y']['w_1'] * (eps_tgt - eps_e) * ~inpainting_mask - model_kwargs['y']['w_2'] * eps_e * ~inpainting_mask
            alpha_cur = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
            alpha_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, x.shape)
            sigma_t = ((1 - alpha_prev)/(1 - alpha_cur)).sqrt() * (1 - alpha_cur / alpha_prev).sqrt()
            
            x = x + sigma_t * delta_x
            old_x = x.detach().clone()
            model_kwargs['y']['text_scale'] = text_scale_temp.detach().clone() 
            model_output = get_conv_model_output(x, t, model_kwargs)
        ...
        if inpainting_util.requires_alm(model_kwargs, t):
            alpha_cur = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
            alpha_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, x.shape)
            sigma_t = ((1 - alpha_prev)/(1 - alpha_cur)).sqrt() * (1 - alpha_cur / alpha_prev).sqrt()
            preservation = model_kwargs['y']['w_1'] * (-1) * inpainting_mask * (old_x - source_motion_x_t)
            model_mean = model_mean + preservation * sigma_t
