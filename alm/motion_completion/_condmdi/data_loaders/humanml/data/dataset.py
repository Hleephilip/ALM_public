...
from alm.motion_completion._condmdi.data_loaders.humanml.utils.word_vectorizer import WordVectorizer
from alm.motion_completion._condmdi.data_loaders.humanml.utils.get_opt import get_opt
from alm.motion_completion._condmdi.data_loaders.humanml.common.quaternion import qinv, qrot
from alm.motion_completion._condmdi.data_loaders.humanml.scripts.motion_process import recover_from_ric, extract_features
from alm.motion_completion._condmdi.data_loaders.humanml.utils.paramUtil import *
from alm.motion_completion._condmdi.data_loaders.humanml.common.skeleton import Skeleton
...
class HumanML3D(data.Dataset):
    def __init__(self,
        ...
        runtime_paths = get_motion_paths()
        dataset_opt_path = str(
            runtime_paths.dataset_resources / 'humanml_opt.txt'
            if datapath is None else os.path.abspath(os.path.expanduser(datapath))
        )
        ...
        opt.meta_dir = str(runtime_paths.dataset_resources)
        opt.checkpoints_dir = str(runtime_paths.evaluator_root)
        ...
        if self.use_rand_proj:
            if self.random_proj_scale == 10:
                # NOTE: legacy code
                proj_matrix_dir = str(runtime_paths.dataset_resources)
            else:
        ...
        if mode == 'text_only':
            ...
            self.w_vectorizer = WordVectorizer(
                str(runtime_paths.glove_root), 'our_vab'
            )
            print(
                f't2m dataset aug: {self.augment_type} std_scale_shift: {self.std_scale_shift}'
            )
        ...
        data_dir = runtime_paths.dataset_resources / '000021.npy'
    ...
    def motion_to_abs_data(self, motion, model):
        ...
        for ii in range(len(motion)):
            ...
            from alm.motion_completion._condmdi.data_loaders.humanml.scripts.motion_process import recover_root_rot_pos
...
class KIT(HumanML3D):
    def __init__(self,
                 ...
                 **kwargs):
        if datapath is None:
            datapath = str(get_motion_paths().dataset_resources / 'kit_opt.txt')
