from alm.motion_completion._condmdi.utils.parser_util import our_eval_args # , evaluation_parser
from alm.motion_completion._condmdi.utils.fixseed import fixseed
from datetime import datetime
from alm.motion_completion._condmdi.data_loaders.humanml.motion_loaders.model_motion_loaders import get_mdm_loader, get_mdm_loader_ours  # get_motion_loader
from alm.motion_completion._condmdi.data_loaders.humanml.utils.metrics import *
from alm.motion_completion._condmdi.data_loaders.humanml.networks.evaluator_wrapper import EvaluatorMDMWrapper
from collections import OrderedDict
from alm.motion_completion._condmdi.data_loaders.humanml.scripts.motion_process import *
from alm.motion_completion._condmdi.data_loaders.humanml.utils.utils import *
from alm.motion_completion._condmdi.utils.model_util import create_model_and_diffusion, load_model_wo_clip, load_saved_model
from alm.motion_completion._condmdi.diffusion import logger
from alm.motion_completion._condmdi.utils import dist_util
from alm.motion_completion._condmdi.data_loaders.get_data import DatasetConfig, get_dataset_loader
from alm.motion_completion._condmdi.model.cfg_sampler import ClassifierFreeSampleModel
from alm.motion_completion.paths import get_motion_paths
import copy
...
def load_traj_model(data):
    ...
    traj_model_path = str(
        get_motion_paths().checkpoint.parent.parent
        / "traj_unet_adazero_swxs_eps_abs_fp16_clipwd_224"
        / "model000062500.pt"
    )
...
if __name__ == '__main__':
    ...
    args = our_eval_args()
    assert args.alm, 'This entry point evaluates ALM motion completion.'
    assert args.diffusion_steps == 1000 and not args.use_ddim, \
        'ALM motion completion uses the checkpoint 1,000-step DDPM schedule.'
    ...
    evaluation_root = (
        ...
        if args.output_dir
        else os.path.join(get_motion_paths().output_root, 'evaluation', name)
    )
    os.makedirs(evaluation_root, exist_ok=True)
    log_file = os.path.join(
        evaluation_root,
        'eval_humanml_cond_{}_{}_{}'.format(name, niter, if_ddim),
    )
    save_dir = os.path.join(evaluation_root, f'eval_cond_{niter}{if_ddim}')
    ...
    log_file += f'_{args.edit_mode}_alm={args.alm}_w_1={args.w_1}_w_2={args.w_2}_T={args.transition_length}.log'
