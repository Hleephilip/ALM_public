...
import numpy as np
import random
from alm.motion_completion._condmdi.data_loaders import humanml_utils, amass_utils
...
def get_keyframes_mask(data, lengths, edit_mode='benchmark_sparse', trans_length=10, feature_mode='pos_rot_vel', get_joint_mask=False, n_keyframes=5):
    ...
    elif edit_mode == 'middle_half':
        # Observe frames: first quarter and last quarter frames
        # used for inference
        for i, length in enumerate(lengths.cpu().numpy()):
            length = int(length)
            gt_indices = np.array(range(length//4))
            gt_indices_2 = np.array(range(length*3//4, length))
            obs_joint_mask[i, :, :, gt_indices] = True  # set keyframes
            obs_joint_mask[i, :, :, gt_indices_2] = True  # set keyframes

    elif edit_mode == 'first_half':
        # Observe frames: last half frames
        # used for inference
        for i, length in enumerate(lengths.cpu().numpy()):
            length = int(length)
            gt_indices = np.array(range(length//2, length))
            obs_joint_mask[i, :, :, gt_indices] = True  # set keyframes

    elif edit_mode == 'last_half':
        # Observe frames: first half
        # used for inference
        for i, length in enumerate(lengths.cpu().numpy()):
            length = int(length)
            gt_indices = np.array(range(length//2))
            obs_joint_mask[i, :, :, gt_indices] = True  # set keyframes
...
def relative_to_global(data):
    from alm.motion_completion._condmdi.data_loaders.humanml.scripts.motion_process import recover_root_ang_pos
    output = data.clone()
...
def undo_recover_root_rot_pos(data):
    from alm.motion_completion._condmdi.data_loaders.humanml.common.quaternion import qrot
...
def requires_alm(model_kwargs, denoising_step):
    if 'alm' not in model_kwargs['y'].keys():
        return False
    if model_kwargs['y']['alm']:
        return True
    return False
